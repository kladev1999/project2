...
const MixTable = (table_ID, dateData) => {
  return axios.get(
    "http://localhost:8080/totalOrder/getMixTable/" + table_ID + "/" + dateData,
    { headers: authHeader() }
  );
};

const getTableDatetime = (dateData) => {
  return axios.get("http://localhost:8080/table/getDatetime/" + dateData, {
    headers: authHeader(),
  });
};
...
const updateTable = (table_ID, table) => {
  return axios.put(
    "http://localhost:8080/table/updateTable/" + table_ID,
    table,
    { headers: authHeader() }
  );
};
...
const getMoveTable = () => {
  return axios.get(
    "http://localhost:8080/table/getmoveTable/",
    { headers: authHeader() }
  );
};
...
const getTotalOrder_ID = (table_ID) => {
  return axios.get("http://localhost:8080/table/getTotalOrderID/" + table_ID, {
    headers: authHeader(),
  });
};
const TableService = {
