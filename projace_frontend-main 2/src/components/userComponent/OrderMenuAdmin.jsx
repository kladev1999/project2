...
function OrderMenuAdmin() {
  ...
  const handleTabSelect = (eventKey) => {
    ...
    searchInput(typeMenu[eventKey].typeMenu_Name);
  };
  ...
  const addOrderMenu = (e) => {
    ...
    setTimeout(() => {
      ...
      if (menu_ID.length === 0) {
        ...
        navigate(
          "/ListTotalOrderMenu/" + compoSite + "/" + totalOrder_ID + "/" + "0"
        );
      }
      setIsLoading(false);
    }, 500);
  ...
  const filteredTypeMenu = selectedTab
    ? typeMenu.filter((menu) => menu.typeMenu_Name === selectedTab)
    : typeMenu;
              ...
              <div className="text-center" style={{ marginTop: "10px" }}>
                <h3>
                  ...
                  <span className="cart__subtotal">
                    {" "}
                    {Intl.NumberFormat().format(TotalPrice)} บาท.
                  </span>
                </h3>
                      ...
                      <li className="list-group-item">
                        <div className="form-group mb-2">
                          ...
                          <ul
                            ...
                            role="tablist"
                          >
                            {typeMenu.map((typeMenus, index) => (
                              <li className="nav-item" key={index}>
                                <a
                                  className={
                                    "nav-link" +
                                    (typeMenu.typeMenu_ID === typeMenus.typeMenu_ID
                                      ? " active"
                                      : "")
                                  }
                                  aria-selected={typeMenus.typeMenu_Name === typeMenus.typeMenu_Name}
                                  style={{ color: "black" }}
                                  id={`tab-${index}`}
                                  data-toggle="tab"
                                  href={`#typeMenus-${index}`}
                                  role="tab"
                                  aria-controls={`typeMenus-${index}`}
                                  onClick={() =>
                                    searchInput(typeMenus.typeMenu_Name)
                                  }
                                >
                                  {typeMenus.typeMenu_Name}
                                </a>
                              </li>
                            ))}
                          </ul>
