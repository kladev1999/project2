...
public interface Total_OrderRepository extends JpaRepository<Total_Order, Integer> {
	...
	@Query(value = "SELECT * "
			+ "FROM total_order "
			+ "WHERE table_id NOT IN (SELECT table_id FROM total_order "
			+ "WHERE compo_site = :compo_site) "
			+ "AND total_order_status = 0 "
			+ "AND discount_id is null "
			+ "AND total_order_time_stamp "
			+ "like :Datetime%", nativeQuery = true)
	...
	@Query(value = "SELECT *\n"
			+ "FROM total_order\n"
			+ "WHERE emp_id = :Emp_id\n"
			+ "  AND total_order_time_stamp LIKE :Datetime%\n"
			+ "ORDER BY total_order_status ASC;\n"
			+ "", nativeQuery = true)
	...
	@Modifying
	@Transactional
	@Query(value = "UPDATE total_order SET total_order.total_price = :totalPrice "
			+ "WHERE total_order.total_order_id = :totalOrderID"
			, nativeQuery = true)
